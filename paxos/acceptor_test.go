...
func newPosLogEntries(from LogPos, entries ...LogEntry) []LogEntry {
...
func (s *acceptorLogicTest) newCmd(pos LogPos, cmd string) LogEntry {
	return NewCmdLogEntry(pos, s.currentTerm.ToInf(), []byte(cmd), s.currentTerm)
}

func (s *acceptorLogicTest) newCmdInf(pos LogPos, cmd string) LogEntry {
	return NewCmdLogEntry(pos, InfiniteTerm{}, []byte(cmd), s.currentTerm)
}

func (s *acceptorLogicTest) newCmdTerm(pos LogPos, term TermNum, cmd string) LogEntry {
	return NewCmdLogEntry(pos, term.ToInf(), []byte(cmd), term)
}
...
func TestAcceptorLogic__GetEntriesWithWait(t *testing.T) {
	...
	assert.Equal(t, []LogEntry{
		s.initMemberLogEntry(),
		s.newCmdInf(2, "cmd test 02"),
		s.newCmdInf(3, "cmd test 03"),
	}, output.Entries)
	...
	synctest.Test(t, func(t *testing.T) {
		...
		assert.Equal(t, []LogEntry{
			s.newCmdInf(5, "cmd test 05"),
			s.newCmdInf(6, "cmd test 06"),
		}, output.Entries)
